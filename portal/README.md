...
Vite + TypeScript micro-frontend for the faros quickstart provider. Built
into `dist/` and embedded into the provider binary via `//go:embed` (see
`../assets.go`). The faros hub proxies requests under
...
- `src/main.ts` — entry script the portal loads as a one-shot `<script>`.
  Registers the `<faros-provider-quickstart>` custom element.
...
the `farosContext` property setter, the `faros-navigate` CustomEvent)
stays unchanged.
